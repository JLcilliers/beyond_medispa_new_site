...
import DermalFillerContent from "./DermalFillerContent";
import { SEOHead } from "../../../SEO/SEOHead";

export default function DermalFillerTinaPage() {
    ...
    const fetchContent = async () => {
      try {
        const result = await client.queries.treatment({
          relativePath: "dermal-filler.json",
        });
  ...
  return (
    <>
      <SEOHead
        ...
        pagePath="/procedures/injectables/dermal-filler"
        treatmentCategory={treatmentData.category}
        location={treatmentData.location}
      />
      <DermalFillerContent data={graphQLResponse} />
    </>
  );
}
