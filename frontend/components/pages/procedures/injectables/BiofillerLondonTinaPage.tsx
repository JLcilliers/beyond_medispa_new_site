...
import BiofillerLondonContent from "./BiofillerLondonContent";
import { SEOHead } from "../../../SEO/SEOHead";

export default function BiofillerLondonTinaPage() {
    ...
    const fetchContent = async () => {
      try {
        const result = await client.queries.treatment({
          relativePath: "biofiller-london.json",
        });
  ...
  return (
    <>
      <SEOHead
        ...
        pagePath="/procedures/injectables/biofiller-london"
        treatmentCategory={treatmentData.category}
        location={treatmentData.location}
      />
      <BiofillerLondonContent data={graphQLResponse} />
    </>
  );
}
