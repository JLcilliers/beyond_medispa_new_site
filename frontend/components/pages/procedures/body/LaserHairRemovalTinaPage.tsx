...
import LaserHairRemovalContent from "./LaserHairRemovalContent";
import { SEOHead } from "../../../SEO/SEOHead";

export default function LaserHairRemovalTinaPage() {
    ...
    const fetchContent = async () => {
      try {
        const result = await client.queries.treatment({
          relativePath: "laser-hair-removal.json",
        });
  ...
  return (
    <>
      <SEOHead
        ...
        pagePath="/procedures/body/laser-hair-removal"
        treatmentCategory={treatmentData.category}
        location={treatmentData.location}
      />
      <LaserHairRemovalContent data={graphQLResponse} />
    </>
  );
}
